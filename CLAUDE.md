...
First, follow rules in the `.cursor/rules/` dir.
...
- Prefer `mcp__team-search__search` for web search
- Prefer `mcp__kindly-papers__get_content` for URL content fetching
- Use sub-agents for web search and content fetching: one agent per query/URL, created on demand, returning concise summaries only. Never run searches directly in the main agent context.

## CodeGraphContext (CGC) — MCP code-graph queries
